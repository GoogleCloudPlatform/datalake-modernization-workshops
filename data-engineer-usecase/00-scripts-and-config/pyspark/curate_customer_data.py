...
#Creating a database in the metastore
spark.sql(""" CREATE DATABASE IF NOT EXISTS {} """.format(databaseNm));
...
# Persist the augmented customer dataset to metastore by creating a table in metastore
spark.sql("""DROP TABLE IF EXISTS customer_augmented """);

consolidatedDataDF.write.mode("overwrite").option("path", os.path.join(outputGCSURI, "customer_augmented")).saveAsTable("{}.customer_augmented".format(databaseNm))